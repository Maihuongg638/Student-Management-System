...
route.put("/api/updateDepartment", async (req, res) => {
  let body = req.body;
  let data: Department = {
  ...
  let sql = `UPDATE health_department SET DepName="${data.depName}",DepContact="${data.depContact}",DepCity="${data.depCity}",DepDistrict="${data.depDistrict}",DepStreet="${data.depStreet}",Dep_LocateNum="${data.depLocate}" WHERE DepID = "${data.depID}"`;
  try {
...
export { route as updateDepartment };
